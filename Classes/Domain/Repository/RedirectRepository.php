    ...
    public function findAllByFilter(Filter $filter = null)
    {
        ...
        if ($filter) {
            if ($filter->getOrder()) {
                ...
                if ($order === Filter::ORDER_URL) {
                    $newOrderings = $this->defaultOrderings;
                    $newOrderings['domain'] = $filter->getDirection();
                    $newOrderings['url'] = $filter->getDirection();
                } else {
                    $newOrderings[$order] = $filter->getDirection();
                    foreach ($this->defaultOrderings as $key => $direction) {
                        if (!isset($newOrderings[$key])) {
                            $newOrderings[$key] = $direction;
                        }
                    }
                }
